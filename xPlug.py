...
class XPlug(om.MPlug):
            ...
            if self.isCompound():
                if isinstance(inValue, list):
                    for c in xrange(self.numChildren()):
                        result.append(self.set(self.child(c), inValue[c]))
                    return result
                elif type(inValue) == om.MEulerRotation:
                    self.set(self.child(0), inValue.x)
                    self.set(self.child(1), inValue.y)
                    self.set(self.child(2), inValue.z)
                else:
    ...
    def __getattr__(self, name):
        ...
        Raises:
            AttributeError: if is trying to get an child attribute from a not compund MPlug
        Returns:
            mPlug.MPlug
        """
    ...
    def __repr__(self):
        """override repr operator to return node name
        Returns:
            str: node name
        """
        return self.info
